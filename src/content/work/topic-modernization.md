...
approach: I built the pipeline that generates the inputs the new screens are built from. A deterministic stage parses the legacy interview code and produces three artifacts per Topic — the complete screen flow, the language files, and a Markdown migration report. AI prompts then audit those screens for IRS compliance and generate an audit report. Everything flows to the teams creating the new TypeScript / Fuego Player screens. This deterministic-then-AI pattern was proven here first, then reused on the Form Conversion Service.
